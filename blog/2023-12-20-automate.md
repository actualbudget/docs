...
slug: 2024-01-automateyourbudget
...
As a new year approaches so to do new goals.
I'm hoping to maybe help some of you out with potential new years resolutions by making budgeting as easy as possible while still reaching for your goals.
I believe that Actual is a great tool for this whether you have a simple budget or a complex one.

One hurdle that many have with budgeting is that it takes constant work to stay up to date.
Entering transactions, assigning categories, reconciling with your bank and others can hold us back.
Some of those are just the nature of the beast, but some can streamlined.
I hope to give you some pointers and strategies today to reduce some of the stress of budgeting by streamlining the budgeting processes by using goal templates.
...
The goal templates allow you to tell Actual a set of rules and priorities to budget for you.
Isn't that awesome!
...
A more slightly more advanced option would be if you want to make sure you have $100 in your category, but might have some leftover from last month and not need the full $100.
You can do this by using `up to` like: `#template up to $100`.

#### Schedule
If you have taken the time to curate a list of schedules of expected transactions, you can automate budgeting them.
...
To budget some of this months income use: `#template 10% of all income`.
...
1. It tells Actual to not budget more money then shows in your `To Budget`.
2. I tells Actual what order to apply the templates.  This is not the case by default.
...
Currently I budget all, or nearly all, of my available funds with a single click.
...
By calling out both income categories I make sure that I am also pulling part of my savings dividends back into savings.
But wait, _what is that "previous" keyword?_ you may be asking.
Using the "previous" keyword tells Actual to not use this months income, but to use _last_ months income.
This is how I can use both pay yourself first AND month ahead together.
I know that I have enough funds to do this because I held all of my paycheck forward.
You could use something like `#template 30% of available funds`, but that includes things like rollover, which I don't want here.
...
You don't need to space your priorities out like that.
I do it so if I ever decide to add new categories in between, or feel the need to be more granular with my ordering, I have space and don't need to change everything in lower priority levels.
...
If I happened to build up a lot more than 400 the template wont ever allow there to be more than 700 available.
If something crazy happens and I get a big refund or similar that goes back into the grocery category, then the template will even take money out for me.
...
It may just be a simple one like `#template-10 75`.
...
These categories get used semi-regularly, but start to straddle the like between needs and wants.
These types of categories are also where the usefulness of envelope budgeting starts to become the most useful.
...
A sinking fund is a category that purposefully builds up funds for either emergencies or unexpected expenses.
...
This way if I reach the limit, Actual will not add anymore and I have more leftover for fun categories.

#### Other Categories
The next set of categories are a bit tricky.
There is no reason I couldn't use the same strategy as used with the sinking funds, but I don't really want much in these funds.
...
If I spend for those categories, I pull from the funds available in "Misc Expenses" and call it a day.
...
Since I know that I have already allocated towards savings, I have a green light to budget the remainder on  more fun things.
...
To do this I have two template lines at different priorities it looks like:
...
Then gets a second chance to add more after my fun categories by having a higher priority level.
...
If budgeting feels daunting, don't worry.
It is hard to get going and can be hard if things are tight, but taking the time to get on top of things now will give you more financial freedom in the future.
